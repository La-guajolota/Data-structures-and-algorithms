/*
In this recipe, we will learn how to convert a binary number into a hexadecimal number. A
binary number comprises two bits, 0 and 1. To convert a binary number into a hexadecimal
number, we first need to convert the binary number into a decimal number and then
...
int intodecimal(int bin){
    ...
    while(bin >0){//Separador de digitos
        remainder=bin%10;
        push(remainder);
        bin=bin/10;
    }
    ...
    while(top >=0){//Pesos por valor posicional
        j=pop();
